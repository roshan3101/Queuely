  ...
  return (
    <aside className="rounded-[28px] border border-border bg-card p-4 shadow-sm backdrop-blur">
      ...
      <div className="mt-4 grid gap-3">
        <label className="rounded-2xl border border-border bg-zinc-50 px-3 py-2 dark:bg-zinc-900/30">
          ...
          <button onClick={() => createSession()} className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-left text-sm font-medium text-white transition hover:border-cyan-400/30 hover:bg-cyan-400/10">Add task</button>
                <button onClick={() => createSession()} className="rounded-2xl border border-border bg-card px-4 py-3 text-left text-sm font-medium text-foreground transition hover:border-cyan-400/30 hover:bg-cyan-50 dark:bg-zinc-900/30 dark:hover:bg-cyan-400/10">Add task</button>
      </div>
      <div className="mt-4 space-y-2">
        {sessions.map((session: any) => (
          <button key={session.id} onClick={() => setActiveSessionId(session.id)} className={`w-full rounded-2xl border px-4 py-3 text-left transition ${session.id === activeSessionId ? "border-cyan-400/40 bg-cyan-400/10" : "border-white/10 bg-black/20 hover:border-white/20 hover:bg-white/5"}`}>
            <button key={session.id} onClick={() => setActiveSessionId(session.id)} className={`w-full rounded-2xl border px-4 py-3 text-left transition ${session.id === activeSessionId ? "border-cyan-400/40 bg-cyan-50 text-foreground dark:bg-cyan-400/10" : "border-border bg-card hover:border-border hover:bg-muted dark:bg-zinc-900/30 dark:hover:bg-zinc-900/50"}`}>
            <div className="flex items-center justify-between gap-3">
                <span className="truncate text-sm font-medium text-foreground">{session.title}</span>
                <span className="rounded-full border border-border bg-muted px-2 py-0.5 text-[10px] uppercase tracking-[0.24em] text-muted-foreground">{session.status}</span>
            </div>
        ...
        <div className="mt-6 rounded-[22px] border border-border bg-zinc-50 p-4 dark:bg-zinc-900/30">
          <div className="flex items-center justify-between">
            <div>
          ...
          <div className="mt-3">
            <input type="file" onChange={(e:any)=>{const file = e.target.files?.[0]; if(file) uploadFile(file);}} className="block w-full cursor-pointer rounded-xl border border-dashed border-white/15 bg-white/5 px-3 py-2 text-sm text-zinc-300 file:mr-3 file:rounded-lg file:border-0 file:bg-cyan-400 file:px-3 file:py-1.5 file:text-sm file:font-semibold file:text-slate-950" />
                      <input type="file" onChange={(e:any)=>{const file = e.target.files?.[0]; if(file) uploadFile(file);}} className="block w-full cursor-pointer rounded-xl border border-dashed border-border bg-card px-3 py-2 text-sm text-foreground file:mr-3 file:rounded-lg file:border-0 file:bg-cyan-400 file:px-3 file:py-1.5 file:text-sm file:font-semibold file:text-slate-950 dark:bg-zinc-900/30" />
          </div>
          <div className="mt-3 max-h-56 space-y-2 overflow-auto pr-1">
            {files.slice(0, 8).map((file:any) => (
              <div key={file.id} className="rounded-xl border border-border bg-card px-3 py-2">
                ...
                <div className="mt-2 flex gap-2">
                  <button onClick={()=>reindexFilePrompt(file.id)} className="rounded-lg border border-border bg-zinc-50 px-2 py-1 text-[11px] text-foreground transition hover:bg-zinc-100 dark:bg-zinc-900/30 dark:hover:bg-zinc-900/50">Reindex</button>
