...
export default function FileModal({ fileDetails, onClose, onReindex, onDelete, busyAction }: FileModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/60 p-4 backdrop-blur sm:items-center">
      <div className="w-full max-w-3xl overflow-hidden rounded-[28px] border border-border bg-card shadow-2xl shadow-black/30 dark:bg-zinc-950">
          ...
          <div className="space-y-4">
            ...
            <Card>
              ...
              <CardContent className="space-y-2 text-sm text-zinc-300">
                <div className="flex items-center justify-between rounded-2xl border border-border bg-zinc-50 px-4 py-3 dark:bg-zinc-900/30">
                  <span>Current status</span>
                  <Badge variant="secondary">{fileDetails.status}</Badge>
                </div>
                <div className="rounded-2xl border border-dashed border-border bg-zinc-50 px-4 py-3 text-zinc-500 dark:bg-zinc-900/20">
