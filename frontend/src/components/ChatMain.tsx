...
export default function ChatMain({
  ...
  return (
    <section className="rounded-[28px] border border-border bg-card shadow-sm backdrop-blur">
      <div className="flex flex-col gap-3 border-b border-border px-5 py-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <h2 className="text-lg font-semibold text-foreground">{activeSession?.title ?? "Select a task"}</h2>
            <p className="text-sm text-muted-foreground">{isLoading ? "Refreshing..." : activeSession ? `Task ${activeSession.id.slice(0,8)}` : "Create or select a task to continue."}</p>
        </div>
        <div className="flex items-center gap-2 text-xs text-zinc-400">
          <span className="rounded-full border border-border bg-zinc-50 px-3 py-1 dark:bg-zinc-900/40">{messages.length} messages</span>
          <span className="rounded-full border border-border bg-zinc-50 px-3 py-1 dark:bg-zinc-900/40">{isStreaming ? "streaming" : "idle"}</span>
        </div>
      </div>
      ...
      <div className="max-h-[calc(100vh-23rem)] min-h-112 overflow-y-auto px-5 py-5">
        <div className="space-y-4">
          {messages.map((message:any) => (
            <article key={message.id} className={`rounded-3xl border p-4 ${message.role === "user" ? "ml-auto max-w-[85%] border-cyan-400/20 bg-cyan-50 dark:bg-cyan-400/10" : "mr-auto max-w-[92%] border-border bg-zinc-50 dark:bg-zinc-900/40"}`}>
              <div className="mb-3 flex items-center justify-between gap-3">
                <span className="text-[11px] uppercase tracking-[0.28em] text-zinc-400">{message.role}</span>
                    <span className="text-[11px] text-muted-foreground">#{message.sequence_number}</span>
              </div>
          ...
          {!messages.length ? (
            <div className="rounded-3xl border border-dashed border-border bg-zinc-50 p-8 text-center text-sm text-zinc-500 dark:bg-zinc-900/20">No messages yet. Send a prompt to start a streaming assistant reply.</div>
          ) : null}
        </div>
      </div>

      <div className="border-t border-border p-5">
        <div className="rounded-3xl border border-border bg-zinc-50 p-4 dark:bg-zinc-900/30">
