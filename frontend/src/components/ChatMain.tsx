      ...
      <div className="max-h-[calc(100vh-23rem)] min-h-[28rem] overflow-y-auto px-5 py-5">
        <div className="space-y-4">
          {messages.map((message:any) => (
            <article key={message.id} className={`rounded-[24px] border p-4 ${message.role === "user" ? "ml-auto max-w-[85%] border-cyan-400/20 bg-cyan-400/10" : "mr-auto max-w-[92%] border-white/10 bg-black/20"}`}>
          ...
          {!messages.length ? (
            <div className="rounded-[24px] border border-dashed border-white/10 bg-black/10 p-8 text-center text-sm text-zinc-500">No messages yet. Send a prompt to start a streaming assistant reply.</div>
          ) : null}
        </div>
      </div>
