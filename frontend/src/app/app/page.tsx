...
import { clearTokens, readTokens, writeTokens, type TokenState } from "../../lib/authStorage";
...
export default function Home() {
  ...
  useEffect(() => {
    const id = window.setTimeout(() => {
      ...
      // Defer "loaded" until after tokenState is applied to avoid redirect races.
      window.setTimeout(() => setTokensLoaded(true), 0);
    }, 0);
  ...
  useEffect(() => {
    ...
    writeTokens(tokenState);
  }, [tokenState, tokensLoaded]);
  ...
  }, [router, tokenState.accessToken, tokensLoaded]);

  useEffect(() => {
    if (!tokensLoaded || !tokenState.accessToken) return;
    const id = window.setTimeout(() => {
      void apiFetch<ApiResponse<{ email: string }>>(apiBase, tokenState, setTokenState, "/auth/me")
        .then((payload) => setUserEmail(payload.data.email))
        .catch(() => undefined);
    }, 0);
    return () => window.clearTimeout(id);
  }, [apiBase, tokenState, tokensLoaded]);
  ...
  return (
    <main className="min-h-screen bg-[radial-gradient(circle_at_top_left,_rgba(20,184,166,0.18),_transparent_35%),radial-gradient(circle_at_top_right,_rgba(14,165,233,0.14),_transparent_30%),linear-gradient(180deg,_#081018_0%,_#02060b_100%)] text-zinc-100">
      <div className="mx-auto flex min-h-screen max-w-[1800px] flex-col gap-4 px-4 py-4 lg:px-6">
        ...
        {!isAuthed && tokensLoaded ? (
          <div className="rounded-2xl border border-amber-400/20 bg-amber-400/10 px-4 py-3 text-sm text-amber-100">
            You’re not logged in. Redirecting to <a className="underline" href="/login">/login</a>…
          </div>
        ) : null}
