...
export default function TaskLauncherPage() {
    ...
    try {
      if (taskDraft.jobType === "pdf_processing") {
        ...
        if (!sourceFile?.storage_url) {
          throw new Error("Upload or choose a file before launching PDF processing.");
        }
  ...
  return (
    <AppShell title="Add task" subtitle="Upload a file, choose a task, and run it directly" onSignOut={clearTokens}>
      <div className="grid gap-5 xl:grid-cols-[0.9fr_1.1fr]">
        <Card>
          <CardHeader>
            <CardTitle>Task launcher</CardTitle>
            <CardDescription>No session step. Upload a file or pick an existing one, then submit the task immediately.</CardDescription>
          </CardHeader>
          <CardContent className="grid gap-4">
            ...
            <label className="grid gap-2 rounded-2xl border border-white/10 bg-black/20 px-4 py-3">
              ...
              />
              <p className="text-xs text-zinc-500">Uploaded files become available for PDF processing and can be reused by other tasks.</p>
            </label>
            ...
            {taskDraft.jobType === "pdf_processing" ? (
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <label className="flex items-center gap-2 rounded-2xl border border-white/10 bg-black/20 px-4 py-3 text-sm text-zinc-200">
                    <input
                      type="checkbox"
            ...
            {taskDraft.jobType === "report_generation" ? (
              <div className="space-y-3">
                ...
                <label className="grid gap-2 rounded-2xl border border-white/10 bg-black/20 px-4 py-3">
                  <Label className="text-[10px] uppercase tracking-[0.2em] text-zinc-500">Provider model</Label>
                  <Input
                    value={taskDraft.reportProviderModel}
                    onChange={(event) => setTaskDraft((current) => ({ ...current, reportProviderModel: event.target.value }))}
                    className="border-white/10 bg-black/10"
                    placeholder="Optional override"
                  />
                </label>
                <label className="grid gap-2 rounded-2xl border border-white/10 bg-black/20 px-4 py-3">
                  <Label className="text-[10px] uppercase tracking-[0.2em] text-zinc-500">Summary</Label>
                  <Textarea
                    value={taskDraft.reportSummary}
                    onChange={(event) => setTaskDraft((current) => ({ ...current, reportSummary: event.target.value }))}
                    rows={3}
                    className="border-white/10 bg-black/10"
                ...
                <label className="grid gap-2 rounded-2xl border border-white/10 bg-black/20 px-4 py-3">
                  <Label className="text-[10px] uppercase tracking-[0.2em] text-zinc-500">Sections</Label>
                  <Textarea
            ...
            {taskDraft.jobType === "email_sending" ? (
              <div className="space-y-3">
                <label className="grid gap-2 rounded-2xl border border-white/10 bg-black/20 px-4 py-3">
                  <Label className="text-[10px] uppercase tracking-[0.2em] text-zinc-500">To</Label>
                  <Input
                ...
                <label className="grid gap-2 rounded-2xl border border-white/10 bg-black/20 px-4 py-3">
                  <Label className="text-[10px] uppercase tracking-[0.2em] text-zinc-500">Subject</Label>
                  <Input
                ...
                <label className="grid gap-2 rounded-2xl border border-white/10 bg-black/20 px-4 py-3">
                  <Label className="text-[10px] uppercase tracking-[0.2em] text-zinc-500">Body</Label>
                  <Textarea
                ...
                <label className="grid gap-2 rounded-2xl border border-white/10 bg-black/20 px-4 py-3">
                  <Label className="text-[10px] uppercase tracking-[0.2em] text-zinc-500">HTML</Label>
                  <Textarea
                    value={taskDraft.emailHtml}
                    onChange={(event) => setTaskDraft((current) => ({ ...current, emailHtml: event.target.value }))}
                    rows={4}
                    className="border-white/10 bg-black/10"
            ...
            {activeFile ? (
              <div className="rounded-2xl border border-cyan-400/20 bg-cyan-400/10 px-4 py-3 text-sm text-cyan-50">
                Using {activeFile.original_name} from {activeFile.storage_provider ?? "session storage"}.
              </div>
            ) : null}
            ...
            <Button onClick={() => void submitTask()} disabled={!ready || busy} className="w-full">
              {busy ? "Submitting..." : "Start task"}
            </Button>
          </CardContent>
        </Card>
            ...
            <CardContent className="space-y-3">
              {latestResultJob ? (
                ...
                <div className="rounded-2xl border border-dashed border-white/10 bg-black/10 p-6 text-sm text-zinc-500">
                  Launch a task to see its output here.
                </div>
              )}
            </CardContent>
          </Card>
