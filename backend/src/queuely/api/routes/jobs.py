...
def list_jobs(
    ...
    return ApiResponse(data=jobs, request_id=request_id)
...
def get_job(
    ...
    job = get_job_for_user(session=session, user=current_user, job_id=job_id)
...
def cancel_existing_job(
    ...
    result = cancel_job(session=session, user=current_user, job_id=job_id)
